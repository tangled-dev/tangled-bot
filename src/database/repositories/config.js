...
export default class Config {
    ...
    upsertConfig(name, value, type) {
        ...
        return new Promise((resolve, reject) => {
            this.database.run('INSERT INTO config (config_id, config_name, value, type) VALUES (?,?,?,?) ON CONFLICT(config_name) DO UPDATE SET value = excluded.value, type = excluded.type', [
                id,
                name.toLowerCase(),
                value,
                type
    ...
    deleteConfig(name) {
        name = name.toLowerCase();
        return new Promise((resolve, reject) => {
            this.database.run('DELETE FROM config WHERE config_name=?', [name], (err, row) => {
                if(err)
                    return reject(err)
                resolve();
            });
